...
module subnetPrivateEndpoint '../az.net.private.endpoint.bicep' = {
  ...
  params: {
    ...
    serviceResourceId: globalPrivateLinkScope.id
    subnetResourceId: resourceId(vnetSubscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', privateEndpointVnetName, privateEndpointSubnetName)
    privateDnsZoneGroup: {
      privateDNSResourceIds: [
        privatelink_monitor_azure_com.id
        privatelink_oms_opinsights_azure_com.id
        privatelink_ods_opinsights_azure_com.id
        privatelink_agentsvc_azure_automation_net.id
        privatelink_blob_core_cloudapi_net.id
      ]
    }
  }
  dependsOn: [
