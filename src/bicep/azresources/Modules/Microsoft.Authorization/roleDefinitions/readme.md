...
In general, most of the resources under the `Microsoft.Authorization` namespace allows deploying resources at multiple scopes (management groups, subscriptions, resource groups). The `az.auth.role.definition.bicep` root module is simply an orchestrator module that targets sub-modules for different scopes as seen in the parameter usage section. All sub-modules for this namespace have folders that represent the target scope. For example, if the orchestrator module in the [root](az.auth.role.definition.bicep) needs to target 'subscription' level scopes. It will look at the relative path ['/subscription/az.auth.role.definition.sub.bicep'](./subscription/az.auth.role.definition.sub.bicep) and use this sub-module for the actual deployment, while still passing the same parameters from the root module.
...
```bicep
module roledefinition 'yourpath/modules/Microsoft.Authorization.roleDefinitions/subscription/az.auth.role.definition.bicep' = {}
```
...
```bicep
module roleDefinitions './Microsoft.Authorization/roleDefinitions/az.auth.role.definition.bicep' = {
