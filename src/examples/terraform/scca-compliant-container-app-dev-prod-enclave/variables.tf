...
variable "firewall_threat_detection_mode" {
  ...
  validation {
    condition     = var.firewall_threat_detection_mode == "Alert" || var.firewall_threat_detection_mode == "Deny" || var.firewall_threat_detection_mode == "Off"
    error_message = "The Azure Firewall Intrusion Detection mode must be set to [Alert/Deny/Off]."
  }
}
