resource "azurerm_eventhub_authorization_rule" "listen" {
  ...
  name                = try(format("%s-listen", each.value.custom_name), var.use_caf_naming ? data.azurenoopsutils_resource_name.eventhub_auth_rule[format("%s.listen", each.value.hub_name)].result : "listen-default")
...
resource "azurerm_eventhub_authorization_rule" "send" {
  ...
  name                = try(format("%s-send", each.value.custom_name), var.use_caf_naming ? data.azurenoopsutils_resource_name.eventhub_auth_rule[format("%s.send", each.value.hub_name)].result : "send-default")
  namespace_name      = azurerm_eventhub_namespace.eventhub.name
  eventhub_name       = azurerm_eventhub.eventhub[each.value.hub_name].name
  resource_group_name = var.resource_group_name
...
resource "azurerm_eventhub_authorization_rule" "manage" {
  ...
  name                = try(format("%s-manage", each.value.custom_name), var.use_caf_naming ? data.azurenoopsutils_resource_name.eventhub_auth_rule[format("%s.manage", each.value.hub_name)].result : "manage-default")
  namespace_name      = azurerm_eventhub_namespace.eventhub.name
  eventhub_name       = azurerm_eventhub.eventhub[each.value.hub_name].name
  resource_group_name = var.resource_group_name
