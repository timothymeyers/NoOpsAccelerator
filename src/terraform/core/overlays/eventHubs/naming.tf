data "azurecaf_name" "eventhub_namespace" {
  ...
  suffixes      = compact([var.org_name, var.location_short, var.environment, local.name_suffix, var.use_caf_naming ? "" : "eh"])
...
data "azurecaf_name" "consumer_group" {
  ...
  prefixes      = var.name_prefix == "" ? null : [local.name_prefix]
  suffixes      = compact([var.org_name, var.location_short, var.environment, each.key, local.name_suffix, var.use_caf_naming ? "" : "ehcg"])
  use_slug      = var.use_caf_naming
  clean_input   = true
  separator     = "-"
}
