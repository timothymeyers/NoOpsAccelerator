...
module "mod_svcs_logging_storage" {
  ...
  name                = var.svcs_log_storage_account_name
  ...
  tags = merge(var.tags, {
    ...
    description = format("Shared Services Network Resource: %s", var.svcs_log_storage_account_name)
...
#################################################
### STAGE 2: Build out Shared Services Networking  ###
#################################################
...
module "mod_svcs_network" {
  ...
  // VNET Parameters
  vnet_name           = var.svcs_virtual_network_name
  ...
  tags = merge(var.tags, {
    DeployedBy  = format("AzureNoOpsTF [%s]", terraform.workspace)
    description = format("Shared Services Network Resource: %s", var.svcs_virtual_network_name)
  }) # Tags to be applied to all resources
}
...
module "mod_svcs_subnet" {
  ...
  tags = merge(var.tags, {
    DeployedBy  = format("AzureNoOpsTF [%s]", terraform.workspace)
    description = format("Shared Services Network Resource: %s", var.svcs_subnet_name)
  }) # Tags to be applied to all resources
}
...
module "mod_svcs_routetable" {
  ...
  tags = merge(var.tags, {
    DeployedBy  = format("AzureNoOpsTF [%s]", terraform.workspace)
    description = format("Shared Services Network Resource: %s", "svcs-routetable")
  }) # Tags to be applied to all resources
...
resource "azurerm_subnet_route_table_association" "routetable" {
  ...
  subnet_id      = module.mod_svcs_subnet.subnet_ids[var.svcs_subnet_name]
...
module "mod_svcs_network_nsg" {
  depends_on = [module.mod_svcs_subnet]
  ...
  // NSG Parameters
  name                = var.svcs_network_security_group_name
  resource_group_name = var.resource_group_name
  ...
  tags = merge(var.tags, {
    DeployedBy  = format("AzureNoOpsTF [%s]", terraform.workspace)
    description = format("Shared Services Network Resource: %s", var.svcs_network_security_group_name)
  }) # Tags to be applied to all resources
}
...
resource "azurerm_subnet_network_security_group_association" "nsg" {
  ...
  subnet_id                 = module.mod_svcs_subnet.subnet_ids[var.svcs_subnet_name]
  network_security_group_id = module.mod_svcs_network_nsg.id
}
