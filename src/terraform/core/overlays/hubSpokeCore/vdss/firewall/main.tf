...
module "log_analytics_firewall" {
  ...
  name                       = format("%s-diagnostics", var.firewall_name)
  target_resource_id         = module.mod_firewall.id
  ...
  logs = var.firewall_diagnostics_logs

  // Log Analytics Metrics
  metrics = var.firewall_diagnostics_metrics
