...
resource "random_integer" "centos-vm-password" {
  min = 6
  max = 72
}

resource "random_password" "centos-vm-password" {
  length      = random_integer.centos-vm-password.result
  upper       = true
  lower       = true
  numeric     = true
  special     = true
...
module "mod_virtual_machine" {
  ...
  // VM Settings
  name                            = format("%s-centosvm", var.location)
  size                            = var.size
  admin_username                  = var.admin_username
  admin_password                  = var.use_random_password ? random_password.centos-vm-password.result : var.admin_password
