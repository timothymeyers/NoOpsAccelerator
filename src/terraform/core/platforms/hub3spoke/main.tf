...
/*
SUMMARY: Module Example to deploy an SCCA Compliant Platform Hub/ 3 Spoke Landing Zone
...
// Resource Group for the Identity
...
module "mod_id_central_logging" {
  count  = (var.id_subid != "") ? (var.id_subid != var.hub_subid ? 1 : 0) : 0
  source = "../../modules/Microsoft.Insights/diagnosticSettings"

  // Log Analytics Parameters
  name                       = "log-identity-sub-activity-to-${module.mod_logging.laws_name}"
  target_resource_id         = "/subscriptions/${var.id_subid}"
  log_analytics_workspace_id = module.mod_logging.laws_resource_id
  storage_account_id         = module.mod_networking_identity.storage_account_id
...
######################################
## STAGE 5: Identity Networking    ###
######################################

module "mod_networking_identity" {
  providers = { azurerm = azurerm.id }
  depends_on = [
    module.mod_id_resource_group,
    module.mod_networking_hub_firewall
  ]
  source = "../../overlays/hubSpokeCore/vdss/identity"
  ...
  // Identity Networking Settings
  id_virtual_network_name        = local.idVirtualNetworkName
  id_vnet_address_space          = var.id_vnet_address_space
  id_vnet_subnet_address_space   = var.id_vnet_subnet_address_space
  id_log_storage_account_name    = local.idLogStorageAccountName
  id_subnet_name                 = local.idSubnetName
  id_network_security_group_name = local.idNetworkSecurityGroupName
  firewall_private_ip            = module.mod_networking_hub_firewall.private_ip

  // Identity Network Security Group Rules
  id_network_security_group_rules = var.id_network_security_group_rules

  // Identity Resource Locks
  enable_resource_locks = var.enable_services.enable_resource_locks
...
############################################
## STAGE 6: Shared Services Networking   ###
############################################
...
module "mod_hub_to_id_networking_peering" {
  ...
  // Hub Networking Peering Settings
  peering_name_1_to_2 = "${module.mod_networking_hub.virtual_network_name}-to-${module.mod_networking_identity.virtual_network_name}"
  vnet_1_id           = module.mod_networking_hub.virtual_network_id
  vnet_1_name         = module.mod_networking_hub.virtual_network_name
  vnet_1_rg           = module.mod_hub_resource_group.name

  // Identity Networking Peering Settings
  peering_name_2_to_1 = "${module.mod_networking_identity.virtual_network_name}-to-${module.mod_networking_hub.virtual_network_name}"
  vnet_2_id           = module.mod_networking_identity.virtual_network_id
  vnet_2_name         = module.mod_networking_identity.virtual_network_name
  vnet_2_rg           = module.mod_id_resource_group.name
